<template>
  <div>
    ...
    <div class="mb-6 bg-white dark:bg-gray-800 rounded-md p-4 border border-gray-200 dark:border-gray-700">
      <h2 class="text-lg font-medium text-gray-800 dark:text-gray-200 mb-2">代码复杂度分析</h2>
      <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">
        分析JavaScript代码的各项复杂度指标，包括圈复杂度、Halstead度量和维护指数
      </p>
    ...
    <div v-if="result" class="bg-white dark:bg-gray-800 rounded-md p-4 border border-gray-200 dark:border-gray-700">
      <h3 class="text-lg font-medium text-gray-800 dark:text-gray-200 mb-4">分析结果</h3>
      
      <!-- 整体摘要 -->
      <div class="mb-6 bg-gray-50 dark:bg-gray-700 p-4 rounded-md">
        <h4 class="text-md font-medium text-gray-800 dark:text-gray-200 mb-2">整体代码摘要</h4>
        ...
        <div v-if="options.halsteadMetrics" class="mt-4">
          <h5 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Halstead度量</h5>
          <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div class="p-2 bg-white dark:bg-gray-600 rounded-md">
              <div class="text-xs text-gray-500 dark:text-gray-400">程序长度</div>
              <div class="font-medium">{{ result.summary.halstead.length }}</div>
            </div>
            <div class="p-2 bg-white dark:bg-gray-600 rounded-md">
              <div class="text-xs text-gray-500 dark:text-gray-400">词汇量</div>
              <div class="font-medium">{{ result.summary.halstead.vocabulary }}</div>
            </div>
            <div class="p-2 bg-white dark:bg-gray-600 rounded-md">
              <div class="text-xs text-gray-500 dark:text-gray-400">程序体积</div>
              <div class="font-medium">{{ result.summary.halstead.volume.toFixed(2) }}</div>
            </div>
            <div class="p-2 bg-white dark:bg-gray-600 rounded-md">
              <div class="text-xs text-gray-500 dark:text-gray-400">难度</div>
      ...
      <div v-if="options.functionDetails && result.functions.length > 0">
        <h4 class="text-md font-medium text-gray-800 dark:text-gray-200 mb-2">函数级别分析</h4>
      ...
      <div class="mt-6 text-sm text-gray-600 dark:text-gray-400">
        <p class="mb-2 font-medium">复杂度指标解释：</p>
        <ul class="list-disc list-inside space-y-1">
          <li><span class="inline-block w-3 h-3 bg-green-500 rounded-full mr-1"></span> 低复杂度 - 代码简单清晰，易于理解和维护</li>
          <li><span class="inline-block w-3 h-3 bg-yellow-500 rounded-full mr-1"></span> 中等复杂度 - 代码稍有复杂，但仍然可接受</li>
          <li><span class="inline-block w-3 h-3 bg-red-500 rounded-full mr-1"></span> 高复杂度 - 代码过于复杂，应考虑重构</li>
        </ul>
      </div>
...
async function analyzeCode() {
  ...
  try {
    ...
    for (const func of functions) {
      ...
      const funcName = func.id ? func.id.name : '(匿名函数)'
    ...
    error.value = `无法分析代码: ${err.message}`
  } finally {
