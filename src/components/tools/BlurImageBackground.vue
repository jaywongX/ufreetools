    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div
          ...
          <input type="file" ref="fileInput" class="hidden" accept="image/png,image/jpeg,image/bmp,image/webp" multiple
            @change="handleFiles" />
          ...
          <span class="text-xs text-gray-400 mt-2">{{ $t('tools.blur-image-background.supported') }} · {{
            $t('tools.blur-image-background.pasteTip') }}</span>
        </div>

        <!-- 示例、摄像头、URL -->
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
          <button class="btn btn-secondary" @click="loadSample">{{ $t('tools.blur-image-background.loadSample')
            }}</button>
        ...
        <div class="mt-3">
          ...
          <div class="flex gap-2">
            <input v-model="urlInput" :placeholder="$t('tools.blur-image-background.urlPlaceholder')"
              class="flex-1 border rounded px-3 py-2 bg-white dark:bg-gray-700" />
            <button class="btn btn-secondary" @click="importUrl">{{ $t('tools.blur-image-background.importUrl')
              }}</button>
          </div>
        </div>
        ...
        <div v-if="inputs.length" class="mt-4 space-y-3">
          <div class="flex justify-between items-center">
            ...
            <button class="text-sm text-red-500 hover:text-red-600" @click="clearAll">{{
              $t('tools.blur-image-background.clearAll') }}</button>
          </div>

          <div class="flex items-center gap-2 text-sm">
            <button class="btn btn-sm" @click="selectAll(true)">{{ $t('tools.blur-image-background.selectAll')
              }}</button>
            <button class="btn btn-sm" @click="selectAll(false)">{{ $t('tools.blur-image-background.deselectAll')
              }}</button>
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 max-h-[320px] overflow-y-auto p-1">
            <div v-for="(img, idx) in inputs" :key="img.id"
              ...
              <div class="absolute top-1 left-1 flex gap-1">
                ...
                <button class="btn btn-xs" @click.stop="moveUp(idx)">{{ $t('tools.blur-image-background.orderUp')
                  }}</button>
                <button class="btn btn-xs" @click.stop="moveDown(idx)">{{ $t('tools.blur-image-background.orderDown')
                  }}</button>
              </div>
              <button class="absolute top-1 right-1 text-white bg-black/50 hover:bg-black/70 rounded px-1"
                @click.stop="remove(idx)">×</button>
            </div>
          </div>
        </div>
        ...
        <div class="mt-4 space-y-4">
          ...
          <div class="flex flex-wrap gap-4">
            <label class="flex items-center gap-2"><input type="radio" value="gaussian" v-model="cfg.blurType" />{{
              $t('tools.blur-image-background.blurTypes.gaussian') }}</label>
            <label class="flex items-center gap-2"><input type="radio" value="lens" v-model="cfg.blurType" />{{
              $t('tools.blur-image-background.blurTypes.lens') }}</label>
            <label class="flex items-center gap-2"><input type="radio" value="motion" v-model="cfg.blurType" />{{
              $t('tools.blur-image-background.blurTypes.motion') }}</label>
            <label class="flex items-center gap-2"><input type="radio" value="radial" v-model="cfg.blurType" />{{
              $t('tools.blur-image-background.blurTypes.radial') }}</label>
            <label class="flex items-center gap-2"><input type="radio" value="bokeh" v-model="cfg.blurType" />{{
              $t('tools.blur-image-background.blurTypes.bokeh') }}</label>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <label class="text-sm" v-if="cfg.blurType === 'motion'">{{ $t('tools.blur-image-background.angle') }}: {{
              cfg.angle }}
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="text-sm"><input type="checkbox" v-model="cfg.enableSeg" /> {{
              $t('tools.blur-image-background.enableSeg') }}</label>
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="text-sm">
              ...
              <select v-model="cfg.preset" class="w-full border rounded px-2 py-1 bg-white dark:bg-gray-700">
                ...
                <option value="facebook1200x630">{{ $t('tools.blur-image-background.presets.facebook1200x630') }}
                </option>
            ...
            <label class="text-sm" v-if="cfg.preset === 'custom'">
              ...
              <input type="number" min="16" v-model.number="cfg.customW"
                class="w-full border rounded px-2 py-1 bg-white dark:bg-gray-700" />
            </label>
            <label class="text-sm" v-if="cfg.preset === 'custom'">
              {{ $t('tools.blur-image-background.customHeight') }}
              <input type="number" min="16" v-model.number="cfg.customH"
                class="w-full border rounded px-2 py-1 bg-white dark:bg-gray-700" />
            </label>
            <label class="text-sm">
              {{ $t('tools.blur-image-background.dpi') }}
              <input type="number" min="72" max="300" v-model.number="cfg.dpi"
                class="w-full border rounded px-2 py-1 bg-white dark:bg-gray-700" />
            </label>
          </div>
        </div>
          ...
          <div v-for="(img, idx) in outputs" :key="idx" class="border rounded-lg p-4 bg-white dark:bg-gray-800">
            ...
            <div class="relative group">
              ...
              <div class="absolute top-2 right-2 flex space-x-2">
                <button class="icon-btn" @click="zoomIn(idx)"
                  :aria-label="$t('tools.blur-image-background.zoomIn')">+</button>
                <button class="icon-btn" @click="zoomOut(idx)"
                  :aria-label="$t('tools.blur-image-background.zoomOut')">-</button>
                <button class="icon-btn" @click="resetView(idx)"
                  :aria-label="$t('tools.blur-image-background.resetView')">↺</button>
              </div>
            </div>
            ...
            <div class="mt-4 space-y-2">
              <div class="flex flex-wrap gap-4">
                <label class="flex items-center space-x-2"><input type="radio" v-model="img.exportType"
                    value="image/png" /> <span>PNG</span></label>
                <label class="flex items-center space-x-2"><input type="radio" v-model="img.exportType"
                    value="image/jpeg" /> <span>JPG</span></label>
                <label class="flex items-center space-x-2"><input type="radio" v-model="img.exportType"
                    value="image/webp" /> <span>WEBP</span></label>
              </div>
              <div v-if="img.exportType !== 'image/png'">
                <label class="text-sm">{{ $t('tools.blur-image-background.imageQuality') }}: {{
              ...
              <div class="flex gap-2">
                <button class="btn btn-success flex-1" @click="download(idx)">{{
                  $t('tools.blur-image-background.downloadBtn') }}</button>
              </div>
            </div>
          </div>
        </div>
...
<style scoped>
.btn {
  @apply px-4 py-2 rounded font-semibold shadow transition-colors duration-200 flex items-center justify-center bg-gray-600 hover:bg-gray-700 text-white;
}
...
.btn-xs {
  @apply px-1 py-0.5 text-[10px] bg-gray-700 hover:bg-gray-800 text-white rounded;
}
...
.icon-btn {
  @apply p-1 bg-white dark:bg-gray-700 rounded-full shadow-md hover:bg-gray-100 dark:hover:bg-gray-600;
}
