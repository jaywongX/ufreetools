    ...
    <div class="w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <h2 class="text-xl font-bold mb-2">{{ $t('tools.stl-to-glb-converter.inputTitle') }}</h2>
                ...
                <div class="border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center bg-white dark:bg-gray-800 cursor-pointer hover:border-blue-500 transition-colors"
                    ...
                    <span class="text-gray-500">{{ $t('tools.stl-to-glb-converter.dragTip') }}</span>
                    <span class="text-xs text-gray-400 mt-2">{{ $t('tools.stl-to-glb-converter.supported') }}</span>
                </div>
                ...
                <div v-if="stlFiles.length" class="mt-4">
                    <div class="flex justify-between items-center mb-2">
                        <h3 class="font-medium">
                            {{ $t('tools.stl-to-glb-converter.fileList') }} ({{ stlFiles.length }})
                        </h3>
                        <button class="text-sm text-red-500 hover:text-red-600" @click="clearFiles">
                            {{ $t('tools.stl-to-glb-converter.clearAll') }}
                        </button>
                    </div>
                ...
                <!-- 转换设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.stl-to-glb-converter.settingsTitle') }}</h3>

                    <!-- 模型名称 -->
                    <div class="mb-4">
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('tools.stl-to-glb-converter.modelNameLabel') }}
                        </label>
                        <input type="text" v-model="modelName"
                            :placeholder="$t('tools.stl-to-glb-converter.modelNamePlaceholder')"
                    ...
                    <div class="mb-4">
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('tools.stl-to-glb-converter.unitLabel') }}
                        </label>
                        <select v-model="unit"
                            class="w-full px-3 py-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600">
                            <option value="millimeter">{{ $t('tools.stl-to-glb-converter.unitMillimeter') }}</option>
                            <option value="centimeter">{{ $t('tools.stl-to-glb-converter.unitCentimeter') }}</option>
                            <option value="meter">{{ $t('tools.stl-to-glb-converter.unitMeter') }}</option>
                            <option value="inch">{{ $t('tools.stl-to-glb-converter.unitInch') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('tools.stl-to-glb-converter.materialColor') }}
                        </label>
                        <div class="flex items-center space-x-2">
                            <input type="color" v-model="materialColor" class="w-10 h-10 border rounded" />
                            <input type="text" v-model="materialColor" class="flex-1 px-3 py-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                    </div>
                </div>
                ...
                <button class="btn btn-primary w-full mt-6"
                    ...
                    <span v-if="!isConverting">
                        {{ stlFiles.length > 1 ? $t('tools.stl-to-glb-converter.convertBtn') + ` (${stlFiles.length})` : $t('tools.stl-to-glb-converter.convertBtn') }}
                    </span>
                    <span v-else>
                        {{ $t('tools.stl-to-glb-converter.converting') }}
                ...
                <div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
                    <h4 class="font-semibold mb-1">{{ $t('tools.stl-to-glb-converter.tipsTitle') }}</h4>
                    <p>{{ $t('tools.stl-to-glb-converter.tipContent') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-2">
                    <h2 class="text-xl font-bold">{{ $t('tools.stl-to-glb-converter.outputTitle') }}</h2>
                    <button v-if="convertedFiles.length > 1" class="btn btn-primary btn-sm" @click="downloadAll">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        {{ $t('tools.stl-to-glb-converter.batchDownload') }}
                    </button>
                </div>
                ...
                <div v-if="convertedFiles.length" class="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-1">
                    <div v-for="(file, idx) in convertedFiles" :key="idx"
                        ...
                        <div class="mb-4">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                                    <p class="text-xs text-gray-500 mb-1">{{ $t('tools.stl-to-glb-converter.originalFile') }}</p>
                                    <p class="font-medium">{{ file.originalName }}</p>
                                    <p class="text-xs text-gray-500">{{ formatFileSize(file.originalSize) }}</p>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                                    <p class="text-xs text-gray-500 mb-1">{{ $t('tools.stl-to-glb-converter.convertedFile') }}</p>
                                    <p class="font-medium">{{ file.name }}</p>
                                    <p class="text-xs text-gray-500">{{ formatFileSize(file.size) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="file.triangleCount" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                                <p>{{ $t('tools.stl-to-glb-converter.triangleCount') }}: {{ file.triangleCount.toLocaleString() }}</p>
                                <p>{{ $t('tools.stl-to-glb-converter.vertexCount') }}: {{ (file.triangleCount * 3).toLocaleString() }}</p>
                            </div>
                        </div>
                        ...
                        <button class="btn btn-success w-full" @click="download(idx)">
                            ...
                            {{ $t('tools.stl-to-glb-converter.downloadBtn') }}
                        </button>
                    </div>
                </div>
                ...
                <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p class="mt-4 text-gray-500">{{ $t('tools.stl-to-glb-converter.noOutput') }}</p>
                </div>
            </div>
        </div>
        <StlToGlbConverterArticle />
    </div>
    <Toast ref="toastRef" />
</template>
...
import StlToGlbConverterArticle from './StlToGlbConverterArticle.vue'
...
const materialColor = ref('#4F46E5')
        ...
        reader.onload = (e) => {
            ...
            try {
                ...
                if (asciiResult.triangleCount > 0) {
                    ...
                    reject(new Error(t('tools.stl-to-glb-converter.parseError')))
                }
            }
        }
        reader.onerror = () => reject(new Error(t('tools.stl-to-glb-converter.readError')))
...
// 转换为GLB格式
async function convertToGlb(triangleCount, triangles, name) {
    // 创建Three.js几何体
...
async function convertFile(file) {
    ...
    const blob = await convertToGlb(triangleCount, triangles, name)

    const url = URL.createObjectURL(blob)
    const outputName = file.name.replace(/\.stli$/i, '') + '.glb'
...
async function convertAll() {
    ...
    try {
        ...
        toastRef.value.show(t('tools.stl-to-glb-converter.convertError'))
    } finally {
...
async function downloadAll() {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = 'converted_glb_files.zip'
    a.click()
