    ...
    <div class="w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <h2 class="text-xl font-bold mb-2">{{ $t('tools.obj-to-3mf-converter.inputTitle') }}</h2>
                ...
                <div class="border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center bg-white dark:bg-gray-800 cursor-pointer hover:border-blue-500 transition-colors"
                    ...
                    <span class="text-gray-500">{{ $t('tools.obj-to-3mf-converter.dragTip') }}</span>
                    <span class="text-xs text-gray-400 mt-2">{{ $t('tools.obj-to-3mf-converter.supported') }}</span>
                </div>

                <!-- 文件列表 -->
                <div v-if="objFiles.length" class="mt-4">
                    <div class="flex justify-between items-center mb-2">
                        <h3 class="font-medium">
                            {{ $t('tools.obj-to-3mf-converter.fileList') }} ({{ objFiles.length }})
                        </h3>
                        <button class="text-sm text-red-500 hover:text-red-600" @click="clearFiles">
                            {{ $t('tools.obj-to-3mf-converter.clearAll') }}
                        </button>
                    </div>
                    <div class="space-y-2 max-h-[300px] overflow-y-auto p-1">
                        <div v-for="(file, idx) in objFiles" :key="idx"
                ...
                <!-- 转换设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.obj-to-3mf-converter.settingsTitle') }}</h3>

                    <!-- 模型名称 -->
                    <div class="mb-4">
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('tools.obj-to-3mf-converter.modelNameLabel') }}
                        </label>
                        <input type="text" v-model="modelName"
                            :placeholder="$t('tools.obj-to-3mf-converter.modelNamePlaceholder')"
                    ...
                    <div class="mb-4">
                        <label class="block text-sm text-gray-700 dark:text-gray-300 mb-1">
                            {{ $t('tools.obj-to-3mf-converter.unitLabel') }}
                        </label>
                        <select v-model="unit"
                            class="w-full px-3 py-2 border rounded text-sm dark:bg-gray-700 dark:border-gray-600">
                            <option value="millimeter">{{ $t('tools.obj-to-3mf-converter.unitMillimeter') }}</option>
                            <option value="centimeter">{{ $t('tools.obj-to-3mf-converter.unitCentimeter') }}</option>
                            <option value="meter">{{ $t('tools.obj-to-3mf-converter.unitMeter') }}</option>
                            <option value="inch">{{ $t('tools.obj-to-3mf-converter.unitInch') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        <label class="flex items-center space-x-2">
                            ...
                            <span class="text-sm text-gray-700 dark:text-gray-300">
                                {{ $t('tools.obj-to-3mf-converter.mergeFiles') }}
                            </span>
                        </label>
                    </div>
                </div>
                ...
                <button class="btn btn-primary w-full mt-6"
                    :disabled="!objFiles.length || isConverting"
                    @click="convertAll">
                    ...
                    <span v-if="!isConverting">
                        {{ objFiles.length > 1 ? $t('tools.obj-to-3mf-converter.convertBtn') + ` (${objFiles.length})` : $t('tools.obj-to-3mf-converter.convertBtn') }}
                    </span>
                    <span v-else>
                        {{ $t('tools.obj-to-3mf-converter.converting') }}
                ...
                <div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
                    <h4 class="font-semibold mb-1">{{ $t('tools.obj-to-3mf-converter.tipsTitle') }}</h4>
                    <p>{{ $t('tools.obj-to-3mf-converter.tipContent') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-2">
                    <h2 class="text-xl font-bold">{{ $t('tools.obj-to-3mf-converter.outputTitle') }}</h2>
                    <button v-if="convertedFiles.length > 1" class="btn btn-primary btn-sm" @click="downloadAll">
                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        {{ $t('tools.obj-to-3mf-converter.batchDownload') }}
                    </button>
                </div>
                ...
                <div v-if="convertedFiles.length" class="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-1">
                    <div v-for="(file, idx) in convertedFiles" :key="idx"
                        ...
                        <div class="mb-4">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm">
                                <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                                    <p class="text-xs text-gray-500 mb-1">{{ $t('tools.obj-to-3mf-converter.originalFile') }}</p>
                                    <p class="font-medium">{{ file.originalName }}</p>
                                    <p class="text-xs text-gray-500">{{ formatFileSize(file.originalSize) }}</p>
                                </div>
                                <div class="bg-gray-50 dark:bg-gray-700 p-3 rounded">
                                    <p class="text-xs text-gray-500 mb-1">{{ $t('tools.obj-to-3mf-converter.convertedFile') }}</p>
                                    <p class="font-medium">{{ file.name }}</p>
                                    <p class="text-xs text-gray-500">{{ formatFileSize(file.size) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="file.triangleCount" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                                <p>{{ $t('tools.obj-to-3mf-converter.triangleCount') }}: {{ file.triangleCount.toLocaleString() }}</p>
                                <p>{{ $t('tools.obj-to-3mf-converter.vertexCount') }}: {{ (file.triangleCount * 3).toLocaleString() }}</p>
                            </div>
                        </div>
                        ...
                        <button class="btn btn-success w-full" @click="download(idx)">
                            ...
                            {{ $t('tools.obj-to-3mf-converter.downloadBtn') }}
                        </button>
                    </div>
                </div>
                ...
                <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p class="mt-4 text-gray-500">{{ $t('tools.obj-to-3mf-converter.noOutput') }}</p>
                </div>
            </div>
        </div>
        <ObjToThreeMfConverterArticle />
    </div>
    <Toast ref="toastRef" />
</template>
...
import ObjToThreeMfConverterArticle from './ObjToThreeMfConverterArticle.vue'
...
function handleFiles(e) {
    ...
    files.forEach(file => {
        if (!file.name.toLowerCase().endsWith('.obj')) return
        objFiles.value.push(file)
    })
        ...
        reader.onload = (e) => {
            ...
            for (const line of lines) {
                ...
                if (trimmed.startsWith('v ')) {
                    ...
                    const parts = trimmed.split(/\s+/)
                    // OBJ面索引从1开始，支持多种格式：f v, f v/vt, f v/vt/vn, f v//vn
                    const faceVertices = []
            ...
            resolve({
                ...
                triangles: faces.map(vertices => ({
                    normal: calculateNormal(vertices[0], vertices[1], vertices[2]),
                    vertices: vertices
                }))
            })
        }
        reader.onerror = () => reject(new Error(t('tools.obj-to-3mf-converter.readError')))
...
<model unit="${unitMap[unit.value] || 'millimeter'}" xml:lang="en-US"
  ...
  <metadata name="Application">UFreetools OBJ to 3MF Converter</metadata>
  <resources>
...
async function convertFile(file) {
    const { triangleCount, triangles } = await parseObj(file)

    const name = modelName.value || file.name.replace(/\.obj$/i, '')
    ...
    const outputName = file.name.replace(/\.obj$/i, '') + '.3mf'
...
async function convertAll() {
    if (!objFiles.value.length) return

    isConverting.value = true
    convertedFiles.value.forEach(f => URL.revokeObjectURL(f.url))
    convertedFiles.value = []
    convertingProgress.value = { current: 0, total: objFiles.value.length }

    try {
        if (mergeFiles.value && objFiles.value.length > 1) {
            // 合并所有OBJ为单个3MF
            ...
            for (const file of objFiles.value) {
                const { triangleCount, triangles } = await parseObj(file)
                for (const tri of triangles) {
                    allTriangles.push({
                        normal: tri.normal,
            ...
            convertedFiles.value = [{
                ...
                name: 'merged.3mf',
                size: blob.size,
                originalName: objFiles.value.map(f => f.name).join(', '),
                originalSize: objFiles.value.reduce((sum, f) => sum + f.size, 0),
                triangleCount: totalCount
            }]
        } else {
            // 逐个转换
            for (let i = 0; i < objFiles.value.length; i++) {
                convertingProgress.value.current = i + 1
                const result = await convertFile(objFiles.value[i])
                convertedFiles.value.push(result)
            }
        }
    } catch (error) {
        console.error('转换失败', error)
        toastRef.value.show(t('tools.obj-to-3mf-converter.convertError'))
    } finally {
