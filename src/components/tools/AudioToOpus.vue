    ...
    <div class="w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <h2 class="text-xl font-bold mb-2">{{ $t('tools.audio-to-opus.inputTitle') }}</h2>
                <div class="border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center cursor-pointer hover:border-blue-500 transition-colors"
                    ...
                    <span class="text-gray-500">{{ $t('tools.audio-to-opus.dragTip') }}</span>
                    <span class="text-xs text-gray-400 mt-2">{{ $t('tools.audio-to-opus.supported') }}</span>
                </div>
                <div class="flex justify-between items-center mt-4">
                    <button class="btn btn-secondary" @click="loadSampleAudio">{{ $t('tools.audio-to-opus.loadSample') }}</button>
                    <button v-if="originalAudios.length" class="text-sm text-red-500" @click="clearAudios">{{ $t('tools.audio-to-opus.clearAll') }}</button>
                </div>
                <div v-if="originalAudios.length" class="mt-4">
                    <h3 class="font-medium mb-2">{{ $t('tools.audio-to-opus.preview') }} ({{ originalAudios.length }})</h3>
                ...
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-opus.bitrateTitle') }}</h3>
                    <select v-model="bitrate" class="w-full px-3 py-2 border rounded dark:bg-gray-700">
                        <option value="16">{{ $t('tools.audio-to-opus.bitrate16') }}</option>
                        <option value="24">{{ $t('tools.audio-to-opus.bitrate24') }}</option>
                        <option value="32">{{ $t('tools.audio-to-opus.bitrate32') }}</option>
                        <option value="48">{{ $t('tools.audio-to-opus.bitrate48') }}</option>
                        <option value="64">{{ $t('tools.audio-to-opus.bitrate64') }}</option>
                        <option value="96">{{ $t('tools.audio-to-opus.bitrate96') }}</option>
                        <option value="128">{{ $t('tools.audio-to-opus.bitrate128') }}</option>
                        <option value="192">{{ $t('tools.audio-to-opus.bitrate192') }}</option>
                        <option value="256">{{ $t('tools.audio-to-opus.bitrate256') }}</option>
                    </select>
                </div>
                <div class="mt-4">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-opus.sampleRateTitle') }}</h3>
                    <select v-model="sampleRate" class="w-full px-3 py-2 border rounded dark:bg-gray-700">
                        <option value="auto">{{ $t('tools.audio-to-opus.sampleRateAuto') }}</option>
                        <option value="8000">{{ $t('tools.audio-to-opus.sampleRate8000') }}</option>
                        <option value="12000">{{ $t('tools.audio-to-opus.sampleRate12000') }}</option>
                        <option value="16000">{{ $t('tools.audio-to-opus.sampleRate16000') }}</option>
                        <option value="24000">{{ $t('tools.audio-to-opus.sampleRate24000') }}</option>
                        <option value="48000">{{ $t('tools.audio-to-opus.sampleRate48000') }}</option>
                    </select>
                </div>
                <div class="mt-4">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-opus.channelsTitle') }}</h3>
                    <select v-model="channels" class="w-full px-3 py-2 border rounded dark:bg-gray-700">
                        <option value="1">{{ $t('tools.audio-to-opus.channelsMono') }}</option>
                        <option value="2">{{ $t('tools.audio-to-opus.channelsStereo') }}</option>
                    </select>
                </div>
                <button class="btn btn-primary w-full mt-6" :disabled="!originalAudios.length || isConverting" @click="convertAll">
                    <span v-if="!isConverting">{{ $t('tools.audio-to-opus.convertBtn') }}</span>
                    <span v-else>{{ $t('tools.audio-to-opus.converting') }}</span>
                </button>
                <div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
                    <h4 class="font-semibold mb-1">{{ $t('tools.audio-to-opus.tipsTitle') }}</h4>
                    <p>{{ $t('tools.audio-to-opus.tipContent') }}</p>
                </div>
            </div>
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-2">
                    <h2 class="text-xl font-bold">{{ $t('tools.audio-to-opus.outputTitle') }}</h2>
                    <button v-if="convertedAudios.length > 1" class="btn btn-primary btn-sm" @click="downloadAll">{{ $t('tools.audio-to-opus.batchDownload') }}</button>
                </div>
                <div v-if="convertedAudios.length" class="space-y-4 max-h-[400px] overflow-y-auto">
                    <div v-for="(audio, idx) in convertedAudios" :key="idx" class="border rounded p-4">
                        ...
                        <p class="text-xs text-gray-500 mb-1">{{ $t('tools.audio-to-opus.originalSize') }}: {{ formatFileSize(audio.originalSize) }}</p>
                        <p class="text-xs text-gray-500 mb-3">{{ $t('tools.audio-to-opus.convertedSize') }}: {{ formatFileSize(audio.convertedSize) }}</p>
                        <button class="btn btn-success w-full" @click="download(idx)">{{ $t('tools.audio-to-opus.downloadBtn') }}</button>
                    </div>
                </div>
                <div v-else class="text-center py-12 bg-gray-50 rounded-lg">
                    <p class="text-gray-500">{{ $t('tools.audio-to-opus.noOutput') }}</p>
                </div>
            </div>
        </div>
        <AudioToOpusArticle />
...
import AudioToOpusArticle from './AudioToOpusArticle.vue'
...
async function convertAll() {
    ...
    for (const audio of originalAudios.value) {
        ...
        const opusBlob = audioBufferToWav(buf) // Simplified: using WAV as placeholder
        convertedAudios.value.push({ url: URL.createObjectURL(opusBlob), blob: opusBlob, name: audio.name.replace(/\.\w+$/, '.opus'), originalSize: audio.file.size, convertedSize: opusBlob.size })
        ctx.close()
    }
...
async function downloadAll() { const zip = new JSZip(); for (const a of convertedAudios.value) zip.file(a.name, a.blob); const url = URL.createObjectURL(await zip.generateAsync({ type: 'blob' })); const a = document.createElement('a'); a.href = url; a.download = 'converted_opus.zip'; a.click() }
