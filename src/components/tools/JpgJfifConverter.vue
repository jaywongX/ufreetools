<template>
    <div class="w-full mx-auto p-0 sm:p-2">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 mb-6">
        <h2 class="text-xl font-bold mb-4">{{ $t('tools.jpg-jfif-converter.name') }}</h2>
        <p class="text-gray-600 dark:text-gray-400 mb-4">{{ $t('tools.jpg-jfif-converter.upload.subtitle') }}</p>
        ...
        >
          <div v-if="!files.length">
            ...
            <p class="mt-4 text-gray-600 dark:text-gray-400">{{ $t('tools.jpg-jfif-converter.upload.dropzone') }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-500 mt-2">{{ $t('tools.jpg-jfif-converter.upload.maxSize') }}</p>
            <p class="text-sm text-gray-500 dark:text-gray-500 mt-1">{{ $t('tools.jpg-jfif-converter.upload.supportedFormats') }}</p>
            <button 
              @click="triggerFileInput" 
              class="mt-4 px-4 py-3 bg-primary text-white rounded-md hover:bg-primary-dark transition"
            >
              {{ $t('tools.jpg-jfif-converter.actions.selectFile') }}
            </button>
            <input 
              ref="fileInput" 
              type="file" 
          ...
          <div v-else>
            <div class="relative max-h-64 overflow-auto mb-4">
              <table class="w-full text-sm text-gray-700 dark:text-gray-300">
                <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0">
                  <tr>
                    <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.fileTable.fileName') }}</th>
                    <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.fileTable.type') }}</th>
                    <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.fileTable.size') }}</th>
                    <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.fileTable.status') }}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(file, index) in files" :key="index" class="border-b border-gray-200 dark:border-gray-600">
                    ...
                    <td class="px-4 py-2">
                      <span v-if="file.status === 'pending'" class="text-blue-500">{{ $t('tools.jpg-jfif-converter.fileTable.pending') }}</span>
                      <span v-else-if="file.status === 'converting'" class="text-yellow-500">{{ $t('tools.jpg-jfif-converter.fileTable.converting') }}</span>
                      <span v-else-if="file.status === 'done'" class="text-green-500">{{ $t('tools.jpg-jfif-converter.fileTable.done') }}</span>
                      <span v-else class="text-red-500">{{ $t('tools.jpg-jfif-converter.fileTable.error') }}</span>
            ...
            <div class="flex space-x-2 justify-center">
              ...
              >
                {{ $t('tools.jpg-jfif-converter.actions.clear') }}
              </button>
              <button 
                @click="triggerFileInput" 
                class="px-3 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md text-sm"
              >
                {{ $t('tools.jpg-jfif-converter.actions.addMore') }}
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-if="files.length > 0" class="mt-6">
          <h3 class="font-bold mb-3">{{ $t('tools.jpg-jfif-converter.settings.title') }}</h3>
          
          <div class="space-y-4">
            <!-- Quality Setting -->
            <div>
              <label class="flex items-center justify-between">
                <span>{{ $t('tools.jpg-jfif-converter.settings.quality.label') }}: {{ quality }}%</span>
            ...
            <div class="flex items-center">
              ...
              <label for="preserveMetadata" class="text-sm">
                {{ $t('tools.jpg-jfif-converter.settings.preserveMetadata') }}
              </label>
            </div>
            ...
            <div>
              <label class="block text-sm font-medium mb-1">
                {{ $t('tools.jpg-jfif-converter.settings.filenameOption') }}
              </label>
              <div class="grid grid-cols-3 gap-2 text-sm">
                ...
                >
                  {{ $t('tools.jpg-jfif-converter.settings.filenameOriginal') }}
                </button>
                <button 
                ...
                >
                  {{ $t('tools.jpg-jfif-converter.settings.filenameConverted') }}
                </button>
                <button 
                ...
                >
                  {{ $t('tools.jpg-jfif-converter.settings.filenameCustom') }}
                </button>
              </div>
              ...
              <div v-if="filenameOption === 'custom'" class="mt-2">
                <input 
                  ...
                  :placeholder="t('tools.jpg-jfif-converter.settings.filenameTemplatePlaceholder')"
                />
                <p class="text-xs text-gray-500 mt-1">
                  {{ $t('tools.jpg-jfif-converter.settings.filenameTemplateHelp') }}
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center mb-4">
              ...
              <label for="autoOptimize" class="text-sm">
                {{ $t('tools.jpg-jfif-converter.settings.quality.auto') }}
              </label>
            </div>
          </div>
        </div>
        ...
        <div class="mt-6">
          ...
          >
            <h3 class="font-bold">{{ $t('tools.jpg-jfif-converter.settings.advanced') }}</h3>
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium mb-2">
                  {{ $t('tools.jpg-jfif-converter.settings.metadata.label') }}
                </label>
                <div class="space-y-2">
                  <div class="flex items-center">
                    <input 
                    ...
                    />
                    <label for="preserveMetadata" class="text-sm">
                      {{ $t('tools.jpg-jfif-converter.settings.metadata.preserve') }}
                    </label>
                  </div>
                  <div class="flex items-center">
                    <input 
                    ...
                    <label for="stripMetadata" class="text-sm">
                      {{ $t('tools.jpg-jfif-converter.settings.metadata.strip') }}
                    </label>
                  </div>
                  <div class="flex items-center">
                    <input 
                    ...
                    <label for="keepBasicMetadata" class="text-sm">
                      {{ $t('tools.jpg-jfif-converter.settings.metadata.keepBasic') }}
                    </label>
                  </div>
                </div>
              </div>
        ...
        <div v-if="files.length > 0" class="mt-6">
          ...
          >
            <span v-if="isConverting">
              ...
              {{ $t('tools.jpg-jfif-converter.actions.converting') }}
            </span>
            <span v-else>
              {{ $t('tools.jpg-jfif-converter.actions.convertAll') }}
            </span>
          </button>
        </div>
      </div>
      ...
      <div v-if="convertedFiles.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6 mb-6">
        <h2 class="text-xl font-bold mb-4">{{ $t('tools.jpg-jfif-converter.results.title') }}</h2>
        
        <div class="max-h-96 overflow-auto mb-4 border border-gray-200 dark:border-gray-700 rounded-lg">
          <table class="w-full text-sm text-gray-700 dark:text-gray-300">
            <thead class="bg-gray-50 dark:bg-gray-700 sticky top-0">
              <tr>
                <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.results.originalName') }}</th>
                <th class="px-4 py-2 text-left">{{ $t('tools.jpg-jfif-converter.results.convertedName') }}</th>
                <th class="px-4 py-2 text-right">{{ $t('tools.jpg-jfif-converter.results.originalSize') }}</th>
                <th class="px-4 py-2 text-right">{{ $t('tools.jpg-jfif-converter.results.convertedSize') }}</th>
                <th class="px-4 py-2 text-center">{{ $t('tools.jpg-jfif-converter.results.actions') }}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <div class="flex space-x-2 justify-center">
          ...
          >
            ...
            {{ $t('tools.jpg-jfif-converter.actions.downloadAll') }}
          </button>
          <button 
            @click="resetResults"
            class="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md"
          >
            {{ $t('tools.jpg-jfif-converter.actions.clearResults') }}
          </button>
        </div>
      </div>
      ...
      <div v-if="convertedFiles.length === 0" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 md:p-6">
        <h2 class="text-xl font-bold mb-4">{{ $t('tools.jpg-jfif-converter.info.title') }}</h2>
        
        <!-- 描述内容 -->
        <div class="prose prose-sm dark:prose-invert mb-6">
          <p>{{ $t('tools.jpg-jfif-converter.info.description') }}</p>
          
          <h3>{{ $t('tools.jpg-jfif-converter.info.whatIs.title') }}</h3>
          <p v-html="t('tools.jpg-jfif-converter.info.whatIs.content')"></p>
          
          <h3>{{ $t('tools.jpg-jfif-converter.info.whyConvert.title') }}</h3>
          <ul>
            <li>{{ $t('tools.jpg-jfif-converter.info.whyConvert.reason1') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.whyConvert.reason2') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.whyConvert.reason3') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.whyConvert.reason4') }}</li>
          </ul>
          
          <h3>{{ $t('tools.jpg-jfif-converter.info.howItWorks.title') }}</h3>
          <p>{{ $t('tools.jpg-jfif-converter.info.howItWorks.content') }}</p>
          
          <!-- 隐私信息 -->
          <h3>{{ $t('tools.jpg-jfif-converter.info.privacy.title') }}</h3>
          <p>{{ $t('tools.jpg-jfif-converter.info.privacy.content') }}</p>
        </div>
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 text-blue-800 dark:text-blue-200">
          <h3 class="font-medium mb-2 flex items-center">
            ...
            {{ $t('tools.jpg-jfif-converter.info.tips.title') }}
          </h3>
          <ul class="list-disc list-inside text-sm space-y-1">
            <li>{{ $t('tools.jpg-jfif-converter.info.tips.tip1') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.tips.tip2') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.tips.tip3') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.tips.tip4') }}</li>
            <li>{{ $t('tools.jpg-jfif-converter.info.tips.tip5') }}</li>
          </ul>
        </div>
      </div>
      
      <!-- SEO 文章内容 -->
      <JpgJfifConverterArticle class="mt-6" />
      ...
      <div v-if="selectedPreviewFile" class="mt-6 border rounded-lg overflow-hidden">
        <div class="flex justify-between bg-gray-50 dark:bg-gray-700 p-3">
          <h3 class="font-bold">{{ $t('tools.jpg-jfif-converter.preview.title') }}</h3>
        ...
        <div class="flex flex-col md:flex-row">
          <div class="flex-1 p-3 border-r border-gray-200 dark:border-gray-600">
            <h4 class="text-center mb-2">{{ $t('tools.jpg-jfif-converter.preview.before') }}</h4>
            <img :src="selectedPreviewFile.originalUrl" class="max-w-full max-h-64 mx-auto" />
          </div>
          <div class="flex-1 p-3">
            <h4 class="text-center mb-2">{{ $t('tools.jpg-jfif-converter.preview.after') }}</h4>
  ...
  const qualityLabel = computed(() => {
    if (quality.value >= 90) return quality.value >= 98 ? 
      t('tools.jpg-jfif-converter.settings.qualityLabels.maximum') : 
      t('tools.jpg-jfif-converter.settings.qualityLabels.high');
    if (quality.value >= 70) return t('tools.jpg-jfif-converter.settings.qualityLabels.good');
    if (quality.value >= 50) return t('tools.jpg-jfif-converter.settings.qualityLabels.medium');
    if (quality.value >= 30) return t('tools.jpg-jfif-converter.settings.qualityLabels.low');
    return t('tools.jpg-jfif-converter.settings.qualityLabels.veryLow');
  });
  ...
  function generateFilename(original) {
    if (filenameOption.value === 'original') {
      return original.replace(/\.[^/.]+$/, '') + '.jfif';
    } else if (filenameOption.value === 'converted') {
      return 'converted-' + original.replace(/\.[^/.]+$/, '') + '.jfif';
    } else {
      return filenameTemplate.value.replace('{name}', original.replace(/\.[^/.]+$/, '')) + '.jfif';
    }
  }
      ...
      return new Promise((resolve, reject) => {
        reader.onload = async (e) => {
          ...
          img.onload = () => {
            ...
            // 使用toBlob将canvas转换为jfif格式的blob
            canvas.toBlob((blob) => {
              // 创建新的转换后文件对象
              const convertedFile = {
                originalName: file.name,  // 添加原文件名
                convertedName: generateFilename(file.name),
                name: generateFilename(file.name),
                originalSize: file.size,
                convertedSize: blob.size,
                originalUrl: URL.createObjectURL(file.file),
                previewUrl: URL.createObjectURL(blob),
                blob: blob
              };
              
              // 添加到转换文件列表
              convertedFiles.value.push(convertedFile);
              // 更新状态为完成
              file.status = 'done';
              resolve();
