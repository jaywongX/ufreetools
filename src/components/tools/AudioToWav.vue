    ...
    <div class="w-full">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                <h2 class="text-xl font-bold mb-2">{{ $t('tools.audio-to-wav.inputTitle') }}</h2>
                ...
                <div class="border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center bg-white dark:bg-gray-800 cursor-pointer hover:border-blue-500 transition-colors"
                    ...
                    <input type="file" ref="fileInput" class="hidden"
                        accept="audio/*,.mp3,.flac,.ogg,.aac,.m4a,.wma,.amr,.aiff,.ape"
                    ...
                    <span class="text-gray-500">{{ $t('tools.audio-to-wav.dragTip') }}</span>
                    <span class="text-xs text-gray-400 mt-2">{{ $t('tools.audio-to-wav.supported') }}</span>
                </div>
                ...
                <div v-if="originalAudios.length" class="mt-4">
                    <h3 class="font-medium mb-2">
                        {{ $t('tools.audio-to-wav.preview') }} ({{ originalAudios.length }})
                    </h3>
                ...
                <!-- 位深度设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-wav.bitrateTitle') }}</h3>
                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            {{ $t('tools.audio-to-wav.bitrateLabel', { bitrate: bitDepth }) }}
                        </label>
                        <input type="range" min="16" max="32" step="8" v-model.number="bitDepth"
                            class="w-full" />
                ...
                <!-- 采样率设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-wav.sampleRateTitle') }}</h3>
                    <select v-model="sampleRate" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600">
                        <option value="auto">{{ $t('tools.audio-to-wav.sampleRateAuto') }}</option>
                        <option value="22050">{{ $t('tools.audio-to-wav.sampleRate22050') }}</option>
                        <option value="44100">{{ $t('tools.audio-to-wav.sampleRate44100') }}</option>
                        <option value="48000">{{ $t('tools.audio-to-wav.sampleRate48000') }}</option>
                        <option value="96000">{{ $t('tools.audio-to-wav.sampleRate96000') }}</option>
                    </select>
                </div>

                <!-- 声道设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-wav.channelTitle') }}</h3>
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" v-model="channelMode" value="auto" />
                            <span class="text-sm">{{ $t('tools.audio-to-wav.channelAuto') }}</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" v-model="channelMode" value="stereo" />
                            <span class="text-sm">{{ $t('tools.audio-to-wav.channelStereo') }}</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" v-model="channelMode" value="mono" />
                            <span class="text-sm">{{ $t('tools.audio-to-wav.channelMono') }}</span>
                        </label>
                    </div>
                </div>
                ...
                <!-- 音量设置 -->
                <div class="mt-6">
                    <h3 class="font-semibold mb-2">{{ $t('tools.audio-to-wav.volumeTitle') }}</h3>
                ...
                <button class="btn btn-primary w-full mt-6"
                    ...
                    <span v-if="!isConverting">
                        {{ originalAudios.length > 1 ? $t('tools.audio-to-wav.convertBtn') + ` (${originalAudios.length}个)` : $t('tools.audio-to-wav.convertBtn') }}
                    </span>
                    <span v-else>
                        {{ $t('tools.audio-to-wav.converting') }}
                ...
                <div class="mt-4 text-xs text-gray-500 dark:text-gray-400">
                    <h4 class="font-semibold mb-1">{{ $t('tools.audio-to-wav.tipsTitle') }}</h4>
                    <p>{{ $t('tools.audio-to-wav.tipContent') }}</p>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                ...
                <div v-if="convertedAudios.length" class="space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto pr-1">
                    <div v-for="(audio, idx) in convertedAudios" :key="idx"
                        ...
                        <div class="grid grid-cols-2 gap-2 text-xs text-gray-600 dark:text-gray-400 mb-4">
                            <div>
                                <span class="font-medium">{{ $t('tools.audio-to-wav.originalSize') }}:</span>
                                {{ formatFileSize(audio.originalSize) }}
                            </div>
                            <div>
                                <span class="font-medium">{{ $t('tools.audio-to-wav.convertedSize') }}:</span>
                                {{ formatFileSize(audio.size) }}
                            </div>
                            <div class="col-span-2">
                                <span class="font-medium">{{ $t('tools.audio-to-wav.compressionRatio') }}:</span>
                                {{ audio.size >= audio.originalSize ? '+' : '' }}{{ Math.round((audio.size / audio.originalSize - 1) * 100) }}%
                            </div>
                        </div>
                        ...
                        <button class="btn btn-success w-full" @click="download(idx)">
                            ...
                            {{ $t('tools.audio-to-wav.downloadBtn') }}
                        </button>
                    </div>
                </div>
                ...
                <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p class="mt-4 text-gray-500">{{ $t('tools.audio-to-wav.noOutput') }}</p>
                </div>
            </div>
        </div>
        <AudioToWavArticle />
...
function handleFiles(e) {
    ...
    files.forEach(file => {
        ...
        const isAudio = /^audio\//i.test(type) || 
            file.name.match(/\.(mp3|flac|ogg|aac|m4a|wma|amr|aiff|ape|wav)$/i)
        if (!isAudio) return
...
async function loadSampleAudio() {
    try {
        ...
        const file = new File([wavBlob], 'sample-audio.mp3', { type: 'audio/mpeg' })
        const url = URL.createObjectURL(file)
        originalAudios.value.push({ file, url, name: file.name })
...
// AudioBuffer转WAV
function audioBufferToWav(buffer, targetBitDepth = 16) {
...
// 转换音频为WAV
async function convertAudioToWav(audioData, index) {
    return new Promise((resolve, reject) => {
        ...
        fetch(audioData.url)
            ...
            .then(renderedBuffer => {
                const wavBlob = audioBufferToWav(renderedBuffer, bitDepth.value)
                ...
                resolve({
                    ...
                    name: audioData.name.replace(/\.\w+$/, '.wav')
                })
...
async function convertAll() {
    ...
    try {
        for (let i = 0; i < originalAudios.value.length; i++) {
            ...
            const result = await convertAudioToWav(originalAudios.value[i], i)
