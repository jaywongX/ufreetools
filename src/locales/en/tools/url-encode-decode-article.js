export default {
  ...
  faq: {
    ...
    a5: 'Performing <strong>URL encode decode in SQL Server</strong> or other database systems typically involves using built-in functions or creating custom functions. SQL Server doesn\'t have built-in URL encoding functions, but you can:<br><br>1. Perform the encoding/decoding in your application code before constructing SQL queries<br>2. Create user-defined functions in SQL. For security reasons, it\'s generally better to handle URL encoding/decoding in application code using proper <strong>C# URL encode decode</strong> methods (for .NET applications) or other language-specific implementations rather than in SQL directly.'
  },
