export default {
    name: '몰타 주소 생성기 - 온라인으로 실제 몰타 주소, 신원 정보 및 연락처 생성',
    description: '무료 온라인 몰타 주소 생성기. 실제 형식의 몰타 주소, 우편번호, 전화번호, ID 카드 번호 및 전체 신원 프로필 생성. 섬/도시별 필터링 지원, JSON/CSV 형식으로 내보내기, 프라이버시 보호를 위해 브라우저에서 완전히 실행.',
    inputTitle: '생성 설정',
    outputTitle: '생성된 결과',
    generateBtn: '데이터 생성',
    loadSampleBtn: '샘플 로드 및 생성',
    clearAll: '모두 지우기',
    noOutput: '생성된 결과 없음',
    preview: '데이터 미리보기',
    generationOptions: '생성 옵션',
    address: '주소',
    street: '거리',
    city: '도시',
    region: '섬',
    zipCode: '우편번호',
    record: '레코드',
    locationTitle: '지리 필터',
    selectRegion: '섬 선택',
    selectCity: '도시 선택',
    allRegions: '모든 섬',
    allCities: '모든 도시',
    addressTypeTitle: '주소 유형',
    addressTypeResidential: '주거 주소',
    addressTypeCommercial: '상업 주소',
    addressTypeVirtual: '가상 사서함',
    addressTypeCampus: '대학교 캠퍼스',
    identityTitle: '신원 정보',
    fullName: '성명',
    gender: '성별',
    genderMale: '남성',
    genderFemale: '여성',
    age: '나이',
    birthDate: '생년월일',
    zodiac: '별자리',
    height: '키',
    weight: '체중',
    contactTitle: '연락처 정보',
    phoneNumber: '전화번호',
    phoneType: '전화 유형',
    phoneTypeMobile: '모바일',
    phoneTypeLandline: '유선전화',
    email: '이메일 주소',
    tempEmail: '임시 이메일',
    creditCardTitle: '신용카드 정보',
    cardType: '카드 유형',
    cardNumber: '카드 번호',
    expiryDate: '만료일',
    cvv: 'CVV',
    bankBin: '은행 BIN',
    careerTitle: '경력 프로필',
    jobTitle: '직위',
    salary: '급여',
    companyName: '회사명',
    companySize: '회사 규모',
    industry: '산업',
    employmentStatus: '고용 상태',
    employmentFullTime: '정규직',
    employmentPartTime: '비정규직',
    idCardTitle: 'ID 카드 번호',
    idCardNumber: 'ID 카드 번호',
    fingerprintTitle: '디지털 지문',
    os: '운영체제',
    osVersion: 'OS 버전',
    browser: '브라우저',
    userAgent: '사용자 에이전트',
    screenResolution: '화면 해상도',
    timezone: '시간대',
    guid: 'GUID',
    ipAddress: 'IP 주소',
    macAddress: 'MAC 주소',
    cookiePolicy: '쿠키 정책',
    exportTitle: '내보내기 형식',
    exportJson: 'JSON 내보내기',
    exportCsv: 'CSV 내보내기',
    downloadBtn: '다운로드',
    copyBtn: '복사',
    viewDetails: '세부 정보 보기',
    cvvLabel: 'CVV',
    batchTitle: '일괄 생성',
    quantityLabel: '수량',
    quantityPlaceholder: '수량 입력 (1-100)',
    generating: '생성 중...',
    generated: '생성 완료',
    copySuccess: '클립보드에 복사됨',
    downloadSuccess: '다운로드 성공',
    invalidQuantity: '1-100 사이의 숫자를 입력하세요',
    resultCount: '{count}개의 레코드 생성됨',
    resultSummary: '결과 요약',
    showAll: '모두 보기',
    collapse: '접기'
};
